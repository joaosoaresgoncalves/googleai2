...
import { generateArticlePDF } from '../services/pdfService.ts';
    ...
    <div className="glass rounded-xl p-6 transition-all hover:scale-[1.01] group border-l-4 border-indigo-500">
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <h3 className="text-xl font-bold mt-2 leading-tight">{article.title || "Untitled Article"}</h3>
        ...
        <div className="flex items-center gap-2 mb-2">
          <div className="flex-1 h-2 bg-slate-800 rounded-full overflow-hidden">
            <div 
              ...
              style={{ width: `${article.importanceScore || 0}%` }}
            />
          </div>
          <span className="text-sm font-bold text-indigo-400">{article.importanceScore || 0}% Relevant</span>
        </div>
        <p className="text-slate-300 text-sm line-clamp-3">{article.importanceReasoning || "No reasoning provided."}</p>
      </div>

      <div className="space-y-3 mb-6">
        <h4 className="text-xs font-bold uppercase text-slate-500">Key Sections</h4>
        {article.sections && article.sections.slice(0, 3).map((s, i) => (
          <div key={i} className="flex items-start gap-2">
            <div className="w-1.5 h-1.5 rounded-full bg-indigo-400 mt-1.5" />
            <span className="text-xs text-slate-400 font-medium line-clamp-1">{s.title}</span>
          </div>
        ))}
        {article.sections && article.sections.length > 3 && (
